...
public class SellDAO {
	...
	public int selectListCount() {
		...
		try {
			...
			pstmt = con.prepareStatement("SELECT count(*) FROM TB_SELLLIST");
	...
	public SellListBean selectProduct(String product_num) throws SQLException {
		...
		sql += " ON TB_PRODUCT.PRODUCT_NUM = TB_PDSTOCK.PRODUCT_NUM WHERE TB_PRODUCT.PRODUCT_NUM =? GROUP BY TB_PDSTOCK.PRODUCT_NUM";
		...
		try {
			...
			if (rs.next()) {
				...
				sell.setSell_yn(rs.getString("sell_yn"));
				sell.setSell_date(rs.getDate("sell_date"));
				sell.setTot_price((rs.getInt("TB_PRODUCT.PRODUCT_PRICE") * rs.getInt("SELL_COUNT")));
